...
class AgendarAtendimentoUI:
    ...
    def agendar():
        ...
        if len(medicos) == 0 or len(atendimentos_livres) == 0:
            ...
            descricoes_especialidades = {
                f"ID: {esp.id} - Nome: {esp.nome}": esp.id
                for esp in View.especialidade_listar()
            }
            descricao_especialidade = st.selectbox(
                "Selecione a especialidade", list(descricoes_especialidades.keys())
            )

            especialidade_escolhida = descricoes_especialidades[descricao_especialidade]

            medicos_escolhidos = []

            for medico in medicos:
                if medico.id_especialidade == especialidade_escolhida:
                    medicos_escolhidos.append(medico)
            descricoes_medicos = {
                f"ID: {med.id} - Nome: {med.nome}": med for med in medicos_escolhidos
            }
