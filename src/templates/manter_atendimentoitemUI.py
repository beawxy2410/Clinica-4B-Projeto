...
class ManterAtendimentoItemUI:
    ...
    def listar():
        ...
        if len(itens) == 0:
            ...
            for obj in itens:
                ...
                atendimento = View.atendimento_listar_id(obj.id_atendimento)
                procedimento = View.procedimento_listar_id(obj.id_procedimento)

                dic.append(
                    {
                        "ID": obj.id,
                        "(ID) Atendimento": f"({atendimento.id}) {atendimento.data} - {atendimento.horario}",
                        "(ID) Procedimento": f"({procedimento.id}) {procedimento.descricao}",
                    }
                )
            df = pd.DataFrame(dic)
    ...
    def atualizar():
        ...
        if len(itens) == 0:
            ...
            id_atendimento = st.number_input(
                "Informe o novo ID do atendimento", op.id_atendimento
            )
            id_procedimento = st.number_input(
                "Informe o novo ID do procedimento", op.id_procedimento
            )
            if st.button("Atualizar"):
                try:
                    View.atendimento_item_atualizar(
                        op.id, id_atendimento, id_procedimento
                    )
    ...
    def excluir():
        ...
        if len(itens) == 0:
            ...
            descricao_para_item = {
                f"ID: {c.id} - Atendimento: {c.id_atendimento} - Procedimento: {c.id_procedimento}": c
                for c in itens
            }
            descricao_escolhida = st.selectbox(
                "Exclusão de item de atendimento", list(descricao_para_item.keys())
            )
