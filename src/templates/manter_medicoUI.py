...
class ManterMedicoUI:
    ...
    def listar():
        ...
        if len(medicos) == 0:
            ...
            dic = []
            for obj in medicos:
                especialidade = View.especialidade_listar_id(obj.id_especialidade)
                dic.append(
                    {
                        "ID": obj.id,
                        "Médico": obj.nome,
                        "(ID) Especialidade": f"({especialidade.id}) {especialidade.nome}",
                    }
                )
            df = pd.DataFrame(dic)
    ...
    def atualizar():
        ...
        if len(medicos) == 0:
            ...
            op = st.selectbox(
                ...
                medicos,
                format_func=lambda x: f"ID: {x.id} - Nome: {x.nome} - Especialidade ID: {x.id_especialidade}",
            )
            nome = st.text_input("Informe o novo nome do médico", value=op.nome)
            id_especialidade = st.number_input(
                "Informe o novo ID da especialidade", value=op.id_especialidade
            )
            if st.button("Atualizar"):
                try:
                    especialidade = View.especialidade_listar_id(id_especialidade)
                    if not especialidade:
                        st.error("Especialidade não encontrada")
                        return
    ...
    def excluir():
        ...
        if len(medicos) == 0:
            ...
            descricao_para_medico = {
                f"ID: {c.id} - Nome: {c.nome} - Especialidade ID: {c.id_especialidade}": c
                for c in medicos
            }
            descricao_escolhida = st.selectbox(
                "Exclusão de médico", list(descricao_para_medico.keys())
            )
